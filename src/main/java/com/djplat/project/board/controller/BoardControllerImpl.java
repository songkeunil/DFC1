...
public class BoardControllerImpl implements BoardController {
	...
	//전체 목록 창
	@Override
	@RequestMapping(value= "/djnboard/listArticles.do", method = {RequestMethod.GET, RequestMethod.POST})
	...
	//공지 목록 창
	@Override
	@RequestMapping(value= "/djnboard/listArticles2.do", method = {RequestMethod.GET, RequestMethod.POST})
	public ModelAndView listArticles2(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		Map articlesMap = boardService.listArticles2(pagingMap);
		articlesMap.put("section", section);
		articlesMap.put("pageNum", pageNum);
	...
	//자료 목록 창
		@Override
		@RequestMapping(value= "/djnboard/listArticles3.do", method = {RequestMethod.GET, RequestMethod.POST})
		public ModelAndView listArticles3(HttpServletRequest request, HttpServletResponse response) throws Exception {
			...
			Map articlesMap = boardService.listArticles3(pagingMap);
			articlesMap.put("section", section);
			articlesMap.put("pageNum", pageNum);
		...
		//뉴스 목록 창
				@Override
				@RequestMapping(value= "/djnboard/listArticles4.do", method = {RequestMethod.GET, RequestMethod.POST})
				public ModelAndView listArticles4(HttpServletRequest request, HttpServletResponse response) throws Exception {
					...
					Map articlesMap = boardService.listArticles4(pagingMap);
					articlesMap.put("section", section);
					articlesMap.put("pageNum", pageNum);
				...
				//설문조사 목록 창
				@Override
				@RequestMapping(value= "/djnboard/listArticles5.do", method = {RequestMethod.GET, RequestMethod.POST})
				public ModelAndView listArticles5(HttpServletRequest request, HttpServletResponse response) throws Exception {
					...
					Map articlesMap = boardService.listArticles5(pagingMap);
					articlesMap.put("section", section);
					articlesMap.put("pageNum", pageNum);
	...
	//글 쓰기 창
	  @Override
	  @RequestMapping(value="/djnboard/addNewArticle.do" ,method = RequestMethod.POST)
	  @ResponseBody
...
//		articleMap.put("parentNO", (parentNO == null ? 0: parentNO));
		...
		try {
			...
			message += " location.href='"+multipartRequest.getContextPath()+"/djnboard/listArticles.do'; ";
			message +=" </script>";
			...
			message +=" location.href='"+multipartRequest.getContextPath()+"/djnboard/articleForm.do'; ";
			message +=" </script>";
			resEnt = new ResponseEntity(message, responseHeaders, HttpStatus.CREATED);
			e.printStackTrace();
		}
	  ...
	  //글 상세창 보기
		@RequestMapping(value="/djnboard/viewArticle.do" ,method = RequestMethod.GET)
	  ...
	  // 수정용 상세창 보기
		@RequestMapping(value="/djnboard/forModShowArticles.do" ,method = RequestMethod.GET)
		public ModelAndView forModShowArticles(@RequestParam("brd_no") int brd_no,
				  HttpServletRequest request, HttpServletResponse response) throws Exception{
			String viewName = (String)request.getAttribute("viewName");
			Map articleMap=boardService.viewArticle(brd_no);
			ModelAndView mav = new ModelAndView();
	  ...
	  //글 수정 
	  @RequestMapping(value="/djnboard/modArticle.do" ,method = RequestMethod.POST)
	  @ResponseBody
	  public ResponseEntity modArticle(MultipartHttpServletRequest multipartRequest,  
	    ...
	    try {
	       ...
	       if (fileList != null && fileList.size() != 0) {
			...
			message += " location.href='" + multipartRequest.getContextPath() + "/djnboard/viewArticle.do?brd_no="
					+ brd_no + "';";
			message += " </script>";
			resEnt = new ResponseEntity(message, responseHeaders, HttpStatus.CREATED);
		} catch (Exception e) {
	...
	private List<String> uploadModArticleFile(MultipartHttpServletRequest multipartRequest) throws Exception {
	  ...
	  //글 삭제
	  @Override
	  @RequestMapping(value = "/djnboard/removeArticle.do" , method = RequestMethod.POST)
	  @ResponseBody
	  public ResponseEntity removeArticle(@RequestParam("brd_no") int brd_no,
		  ...
		  try {
			  boardService.removeArticle(brd_no);
				...
				message += " location.href='"+request.getContextPath()+"/djnboard/listArticles.do';";
				message +=" </script>";
			    resEnt = new ResponseEntity(message, responseHeaders, HttpStatus.CREATED);
	...
	// 수정하기에서 이미지 삭제 기능
		@RequestMapping(value = "/djnboard/removeModFile.do", method = RequestMethod.POST)
		@ResponseBody
	  ...
	  //다운로드
	  @RequestMapping(value = "/djnboard/download.do" , method = RequestMethod.POST)
	  private void Filedownload(@RequestParam("brd_no") int brd_no,
		...
		//창 표시
		@RequestMapping(value = "/djnboard/*Form.do", method =  RequestMethod.GET)
		...
		@RequestMapping(value = "/djnboard/searchArticles.do", method = RequestMethod.GET)
